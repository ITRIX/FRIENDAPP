...
Ext.define('Ext.ux.TouchCalendarEvents', {
  ...
  allowEventDragAndDrop: true,
    ...
    // TODO: For now this must happen when the store is loaded, 'painted' is very late..
    this.calendar.on('painted', function doOnce(){
      this.calendar.removeListener('painted', doOnce); // TODO: Make this work so there's no need for 'evented' property
      if (this.calendar.evented) return;
      this.calendar.evented = true;
      this.calendar.eventStore = Ext.getStore(this.calendar.eventStore);
...
//    this.droppable = new Ext.util.Droppable(this.calendar.element, {
//      /**
...
//    });
  ...
  onEventDropDeactivate: function(droppable, draggable, e, opts){
    if (draggable.el.hasCls(this.eventBarCls)) {
      ...
      this.calendar.element.select('div.' + eventRecord.id).each(function(eventBar){
        eventBar.show();
      }, this);
    }
    },
     ...
     */
        ...
        takenDatePositions = []; // stores 'row positions' that are taken on current date
            ...
            store.filterBy(function(record){
                var startDate = record.get(this.startEventField);
                startDate = me.checkIsDateObject(startDate);
                startDate = startDate.clearTime(true).getTime();
                
                var endDate = record.get(this.endEventField);
                endDate = me.checkIsDateObject(endDate);
                endDate = endDate.clearTime(true).getTime();
            ...
            store.each(function(event){
                // Find any Event Bar record in the EventBarStore for the current Event's record (using id)
                var eventBarIndex = this.eventBarStore.findBy(function(record, id){
                    return record.get('EventID') == event.id;
                }, this);
        ...
        if (this.allowEventDragAndDrop) {
          ...
          new Ext.util.Draggable(eventBar, {
            ...
            onStart: function(e){
              ...
              me.calendar.element.select('div.' + eventRecord.id).each(function(eventBar){
  ...
  onEventDragStart: function(draggable, e){
    ...
    eventBarDoesWrap: function(r){
        var barEndDate = r.get('Date');
        barEndDate = this.checkIsDateObject(barEndDate);
        barEndDate = barEndDate.add(Date.DAY, (r.get('BarLength') - 1));
        
        var endEventField = r.get('Record').get(this.endEventField);
        endEventField = this.checkIsDateObject(endEventField);
        return barEndDate.clearTime(true).getTime() !== endEventField.clearTime(true).getTime();
    },
    /**
    ...
    eventBarHasWrapped: function(r){
        var recordDate = r.get('Date');
        recordDate = this.checkIsDateObject(recordDate);
        
        var startEventDate = r.get('Record').get(this.startEventField);
        startEventDate = this.checkIsDateObject(startEventDate);
        return recordDate.clearTime(true).getTime() !== startEventDate.clearTime(true).getTime();
    },
    ...
    createEventWrapper: function(){
        if (this.calendar.rendered && !this.eventWrapperEl) {
            this.calendar.removeListener('painted', this.createEventWrapper);
            ...
            this.calendar.on(this.eventWrapperEl, 'tap', this.onEventWrapperTap, this, {
    ...
    onEventWrapperTap: function(e, node){
        ...
        if (eventID) {
            ...
            this.eventWrapperEl.select('div.' + eventRecord.id).addCls(this.eventBarSelectedCls);
            var dom = Ext.query('.' + eventRecord.id);
  ...
  getEventsWrapperContainer: function(){
    ...
    /**
     * Get the Event record with the specified eventID (eventID equates to a record's id)
     * @method
     * @private
     ...
     * @method
     * @private
     * @param {String} eventID id of a Model instance
     */
